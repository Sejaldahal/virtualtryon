...
VITON-HD Virtual Try-On Inference Script
Uses existing dataset structure with preprocessed files.

Usage:
    python vitonhd_tryon.py --person 00001_00.jpg --cloth 00001_00.jpg --output result.jpg --dataset_dir "/path/with spaces/dataset"
"""
...
def get_opt():
    ...
    parser = argparse.ArgumentParser(description='VITON-HD Virtual Try-On Inference')
    
    # Required arguments
    parser.add_argument('--person', type=str, required=True, help='Person image filename (e.g., 00001_00.jpg)')
    parser.add_argument('--cloth', type=str, required=True, help='Cloth image filename (e.g., 00001_00.jpg)')
    ...
    parser.add_argument('--alias_checkpoint', type=str, default='alias_final.pth', help='ALIAS model')
...
class VITONHDTryOn:
    def __init__(self, opt):
        ...
        print("Initializing VITON-HD models...")
        ...
        print("VITON-HD models loaded successfully!")
    
    def load_models(self):
        """Load pre-trained model checkpoints"""
        checkpoint_paths = {
            'seg': os.path.join(self.opt.checkpoint_dir, self.opt.seg_checkpoint),
            'gmm': os.path.join(self.opt.checkpoint_dir, self.opt.gmm_checkpoint),
            'alias': os.path.join(self.opt.checkpoint_dir, self.opt.alias_checkpoint)
        }
        ...
        load_checkpoint(self.seg, checkpoint_paths['seg'])
        load_checkpoint(self.gmm, checkpoint_paths['gmm'])
        load_checkpoint(self.alias, checkpoint_paths['alias'])
    
    def generate_tryon(self, person_img_name, cloth_img_name):
        """Generate virtual try-on result using existing dataset structure"""
        print("Loading input data from dataset...")
        ...
        with torch.no_grad():
            ...
            print("Step 1: Generating segmentation...")
            ...
            # Step 3: Generate final try-on result
            print("Step 3: Generating final result...")
            ...
            output = self.alias(torch.cat((data['img_agnostic'], data['pose'], warped_c), dim=1), 
...
def main():
    ...
    print(f"Checkpoint directory: {opt.checkpoint_dir}")
    ...
    if not os.path.exists(opt.dataset_dir):
        print(f"Error: Dataset directory not found: {opt.dataset_dir}")
        return
    ...
    if not os.path.exists(cloth_path):
        print(f"Error: Cloth image not found: {cloth_path}")
        return
    else:
    ...
    if not os.path.exists(opt.checkpoint_dir):
        print(f"Error: Checkpoint directory not found: {opt.checkpoint_dir}")
        print("Please download the pre-trained checkpoints and place them in the checkpoint directory.")
        return
    ...
    try:
        ...
        output_name = f"{person_base}_{cloth_base}.jpg"
        
        save_images(result, [output_name], output_dir)
        print(f"✓ Try-on result saved to: {os.path.join(output_dir, output_name)}")
        
    except Exception as e:
        print(f"Error during try-on generation: {e}")
        import traceback
        traceback.print_exc()
...
# python vitonhd_tryon.py --person 00057_00.jpg --cloth 00094_00.jpg --output result.jpg --dataset_dir "/home/sejal/AI Virtual Tryon/datasets/test"
# use this above code to run
# /home/sejal/AI Virtual Tryon/datasets/
# ├── test/
# │   ├── image/                          # Person images
# │   │   ├── 00001_00.jpg
# │   │   ├── 00002_00.jpg
...
# │   │   ├── 00003_00.jpg
# │   │   └── ...
# │   ├── cloth-mask/                     # Clothing masks (binary)
# │   │   ├── 00001_00.jpg
# │   │   ├── 00002_00.jpg
# │   │   ├── 00003_00.jpg
# │   │   └── ...
...
# │   │   ├── 00001_00_rendered.png
# │   │   ├── 00002_00_rendered.png
# │   │   ├── 00003_00_rendered.png
# │   │   └── ...
# │   ├── openpose-json/                  # Pose keypoints (JSON)
# │   │   ├── 00001_00_keypoints.json
# │   │   ├── 00002_00_keypoints.json
# │   │   ├── 00003_00_keypoints.json
# │   │   └── ...
# │   └── test_pairs.txt                  # Test pairs list
